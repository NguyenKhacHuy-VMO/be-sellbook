...
const { getPagination, getSort, randomNum, filter } = require('../helper/helper');
    ...
    try {
      ...
      if (voucherCode) {
        ...
        if (!voucher || voucher.quantity <= 0) {
          await t.rollback();
          return next(new ErrorResponse(HTTP_CODE.BAD_REQUEST, MESSAGE.VOUCHER_INVALID));
        }

        totalPrice = sum + (sum * (DEFAULT_VALUE.DEFAULT_TAX)) - voucher.discountAmount
  ...
  confirmStauts: async (req, res, next) => {
    ...
    try {
      ...
      if (order.orderStatus === DEFAULT_VALUE.VALUE_CREATE) {
        ...
        await t.rollback();
        return next(new ErrorResponse(HTTP_CODE.BAD_REQUEST, MESSAGE.NOT_CONFIRM))
      }
    } catch (error) {
      await t.rollback();
      return next(new ErrorResponse(HTTP_CODE.BAD_REQUEST, error.message))
  ...
  cancelOrder: async (req, res, next) => {
    ...
    try {
      ...
      if (order.orderStatus > 3) {
        await t.rollback();
        return next(new ErrorResponse(HTTP_CODE.BAD_REQUEST, MESSAGE.NOT_CANCEL))
      }

      if (!order) {
        await t.rollback();
        return next(new ErrorResponse(HTTP_CODE.BAD_REQUEST, MESSAGE.ORDER_IS_CANCEL))
      }
      ...
      for (let i = 0; i < order.OrderDetails.length; i++) {
        ...
        if (order.orderStatus > 0) {
          dataProduct = {
            quantityDisplay: order.OrderDetails[i].Product.quantityDisplay + order.OrderDetails[i].quantity,
            quantity: order.OrderDetails[i].Product.quantity + order.OrderDetails[i].quantity
          }
        } else {
          dataProduct = {
            quantityDisplay: order.OrderDetails[i].Product.quantityDisplay + order.OrderDetails[i].quantity,
